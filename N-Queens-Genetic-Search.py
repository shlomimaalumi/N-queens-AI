...
if __name__ == "__main__":
    ...
    maxFitness = (nq * (nq - 1)) / 2  # represents the maximum number of non-attacking pairs of queens
    population = [random_chromosome(nq) for _ in range(POPULATION_SIZE)] # initial population of chromosomes

    # example of population: [[0, 1, 2, 3, 4, 5, 6, 7], [7, 6, 5, 4, ... ], ...]

    generation = 1
    while maxFitness not in [fitness(chrom, maxFitness) for chrom in population] and generation < 200:
    ...
    if maxFitness in fitnessOfChromosomes:
        ...
        print("\nUnfortunately, we could't find the answer until generation {}. The best answer that the algorithm found was:".format(generation - 1))
        print_board(bestChromosomes)
